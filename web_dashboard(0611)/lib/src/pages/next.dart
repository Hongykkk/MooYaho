...
List<List<Offset>> totalList = [];
List<List<List<Offset>>> totalLists = [];
void ListInit(int value) {
  for (var i = 0; i < 2; i++) {
    totalLists.add(totalList);
  }
}
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void initState() {
    ...
    for (var l in imglist) {
      getUiImage(l, 1000, 800); //height, width순
    }
  }
...
//                  size: Size(img.width.toDouble(), revise_heightimg.height.toDouble())),
        ...
        BoxEditor editor = BoxEditor(image: img);
        return Container(
          // padding: EdgeInsets.all(screenSize.width * 0.05),
          decoration:
              BoxDecoration(border: Border.all(color: Colors.blueAccent)),
              ...
              child: Scrollbar(
                ...
                child: PageView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return _buildImage(
                          new GlobalKey(), imagelist[index], mode);
                    }),
                // children: [
...
class BoxEditor extends CustomPainter {
  ...
  void mybox() {
    List<int> indexList = checkCrossLine(totalList, click);
    //print(indexList);
...
// 선색, image resize
  ...
  void paint(Canvas canvas, Size size) {
    ...
    //print(totalList);
    for (List<Offset> point in totalList) {
      //print("paint : $point");

...
class BoxEditor extends CustomPainter {
  ...
  void mybox() {
    for (List<Offset> points in totalList) {
      if (points[0].dx <= click.dx && points[0].dy <= click.dy) {
        if (points[2].dx >= click.dx && points[2].dy >= click.dy) {
          tempBox = points;
          print("누른좌표가 박스안에 있음 $tempBox");
        }
      }
    }
    return;
